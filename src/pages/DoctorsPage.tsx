...
const DoctorsPage = () => {
  ...
  const { data: doctorInstituteLinks = [] } = useQuery({
    queryKey: ["public-doctor-institute-links"],
    queryFn: async () => {
  ...
  const { data: institutes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("institutes").select("id, name, slug").order("display_order");
      if (error) throw error;
      return data;
    },
  });
                  ...
                  <div className="p-6">
                    {doctor.photo_url ? (
                      <img
                        ...
                        alt={doctor.name}
                        className="w-20 h-20 rounded-2xl object-cover mb-4 border-2 border-border group-hover:border-primary/30 transition-colors"
                      />
                    ) : (
                    ...
                    {docInstitutes.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-3">
                        {docInstitutes.map((inst: any) => (
                          <span key={inst.id} className="inline-block text-xs font-medium text-muted-foreground bg-secondary rounded-full px-3 py-1">
                            {inst.name}
                          </span>
                        ))}
                      </div>
